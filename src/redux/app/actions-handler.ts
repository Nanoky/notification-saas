...
import { AppActions } from "./config.slice";
...
export interface ActionHandler {
    ...
    checkIdempotency(key: string, api: ListenerAPI): boolean;
    clearIdempotency(key: string, api: ListenerAPI): void;
}
...
export class ListenerActionHandler implements ActionHandler {
    ...
    checkIdempotency(key: string, api: ListenerAPI): boolean {
        const inFlight = (api.getState() as RootState).app.inFlight;
        if (inFlight[key]) {
            return true;
        }
        api.dispatch(AppActions.addInFlight(key));
        return false;
    }
    clearIdempotency(key: string, api: ListenerAPI): void {
        api.dispatch(AppActions.removeInFlight(key));
    }
